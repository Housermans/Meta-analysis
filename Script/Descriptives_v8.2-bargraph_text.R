...
data_Jeanine <- d %>%
  filter(DrugType %in% c("Immunomodulator", "Antiangiogenesis")) %>%
  select(Name, DrugType, Year_published_online, Phase)%>%
  arrange(DrugType, Phase)

file_path <- file.path(output, "Jeanine_angio_immuno.xlsx")

#write_xlsx(data_Jeanine, file_path)
...
sum_per_phase <- d_complete %>%
  group_by(Phase) %>%
  summarise(Total_N_inc = sum(as.numeric(N_inc)))
print(sum_per_phase)
...
corplot8 <- ggplot(d_complete, aes(x = Ratio_event_top5, y = CBR_both)) + 
...
#function for plot width Year bargraphs 
adjust_plot_width <- function(plot, name) {
  num_bars <- length(unique(ggplot_build(plot)$data[[1]]$x))
  bar_size <- 200 + num_bars * 130
  legend_title_size <- (nchar(plot$labels$fill))
  legend_name_size <- max(nchar(as.character(plot$data[[2]])), na.rm = TRUE)
  textsize <- ifelse(legend_name_size*20<legend_title_size*30, 30, 20)
  legend_size <- max(legend_name_size, legend_title_size)
  plot_width <- bar_size + legend_size*textsize
  ggsave(file.path(plot_d, name), plot = plot, width = plot_width, height = as.numeric(1200), units = "px")
}
...
targeted_yeargroup <- ggplot(d_complete_ratio, aes(x = year_group, y = ratio, fill = targeted)) +
  ...
  theme_classic() + 
  labs(
    title = "Ratio of trials with preselected patients",
    x = "Year group",
    y = "Ratio",
    fill = "Preselection"
  ) +theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_fill_manual(
    values = c("Targeted" = "#00A88E", "Not targeted" = "white"),
    labels = c("Targeted" = "Preselected", "Not targeted" = "Not preselected")) +
...
#save the plot
ggsave(file.path(plot_d, "Targeted_YearGroup.pdf"), plot=targeted_yeargroup, width=1200, height=1200, units="px")
...
investigational_status_plot <- ggplot(d_complete_ratio_investigation, aes(x = year_group, y = ratio, fill = Investigational_status)) +
  ...
  theme_classic() +
  labs(
    title = "Ratio of trials per investigational status",
    x = "Year group",
    y = "Ratio",
...
# Save the plot
ggsave(file.path(plot_d, "Investigational_Status_YearGroup.pdf"), plot = investigational_status_plot, width=1800, height=1200, units="px")
...
ps_ratio_plot <- ggplot(d_complete_ratio_ps, aes(x = year_group, y = ratio, fill = PS_ratio_cat)) +
  ...
  theme_classic() +
  labs(
    title = "Ratio of trials\nper performance status category",
    x = "Year group",
    y = "Ratio",
    fill = "PS ratio\ncategory"
  ) +  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_fill_manual(values = c("lightblue", "#00A88E", "#C9A8CE"), 
                    labels = c("<0", "0-0.1", ">0.1", "Unknown"), na.value = "white") +
  scale_y_continuous(labels = scales::percent) # You can add your custom colors using scale_fill_manual if needed

ps_ratio_plot

# Save the plot
ggsave(file.path(plot_d, "PS_Ratio_Category_YearGroup.pdf"), plot = ps_ratio_plot, width=1100, height=1200, units="px")
...
d_complete_ratio_age$Med_age_cat
...
med_age_plot <- ggplot(d_complete_ratio_age, aes(x = year_group, y = ratio, fill = Med_age_cat)) +
  ...
  theme_classic() +
  labs(
    title = "Ratio of trials per median age category",
    x = "Year group",
    y = "Ratio",
    fill = "Median age\ncategory"
  ) +  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_fill_manual(values = c("lightblue", "#00A88E", "#C9A8CE", "grey"),                    
                    labels = c("<55", "56-60", "61-65", ">65", "Unknown"), na.value = "white")+
  scale_y_continuous(labels = scales::percent) # You can add your custom colors using scale_fill_manual if needed
...
# Save the plot
ggsave(file.path(plot_d, "Med_Age_Category_YearGroup.pdf"), plot = med_age_plot, width=1100, height=1200, units="px")
...
drugtype_year_plot <- ggplot(d_complete_ratio_drug, aes(x = year_group, y = ratio, fill = DrugType)) +
  ...
  theme_classic() +
  labs(
    title = "Ratio of trials per drug class",
    x = "Year group",
    y = "Ratio",
    fill = "Drug class"
  ) +
...
ggsave(file.path(plot_d, "DrugType_Category_YearGroup.pdf"), plot = drugtype_year_plot, width=1470, height=1200, units="px")
