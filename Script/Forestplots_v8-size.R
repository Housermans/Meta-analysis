...
create_summary_forestplot <- function(data, events_col, n_col,  plot_name, plot_title, category, phase = NULL, Width = 1700, plot_complete = FALSE, use_prediction_intervals = FALSE) {
  ...
  if (use_prediction_intervals) {
    if (events_col == "events_ORR") {
      limits <- c(0, 1)
      breaks <- c(Estimate_overall, seq(0, 0.5, by = 0.5))
    } else if (events_col == "events_CBR") {
      limits <- c(0, 1)
      breaks <- c(Estimate_overall, seq(0, 1, by = 0.5))
      ...
      limits <- c(0, 1)  # Default limits
      breaks <- c(Estimate_overall, seq(0, 1, by = 0.5))
    }  
  } else {
    if (events_col == "events_ORR") {
      limits <- c(0, 0.3)
      breaks <- c(Estimate_overall, seq(0, 0.3, by = 0.15))
    } else if (events_col == "events_CBR") {
      limits <- c(0, 0.7)
      breaks <- c(Estimate_overall, seq(0, 0.7, by = 0.7))
    } else if (events_col == "N_pat_GR3") {
      limits <- c(0,1)
      breaks <- c(Estimate_overall, seq(0, 1, by = 1))
    } else if (events_col == "N_Disc") {
      limits <- c(0, 0.3)
      breaks <- c(Estimate_overall, seq(0, 0.3, by = 0.3))
    } else if (events_col == "N_DLT") {
      limits <- c(0, 0.3)
      breaks <- c(Estimate_overall, seq(0, 0.3, by = 0.3))
    } else {
      limits <- c(0, 0.7)  # Default limits
      breaks <- c(Estimate_overall, seq(0, 0.7, by = 0.7))
    }
  }
  ...
  plot_category <- ggplot(data=merge, aes(y=-index, x=Estimate, xmin=Lower, xmax=Upper)) +
    ...
    scale_x_continuous(breaks = breaks, labels = sprintf("%.2f", breaks),  limits = limits)
  
  
  
  #Put the individual components of the forest plot together
  plot <-plot_grid(cat_n, t1, t2, plot_category, nrow = 1, rel_widths = c(((((max(stringr::str_width(unique(merge$category))))/20)+0.55)*4), 2.5, 3.5, 5))+
      plot_annotation(title=plot_title, theme=theme(plot.title=element_text(hjust=0.5, size = 10)))
  
  Height =  400 + n_category * 100
  Width =  1100 + max(((stringr::str_width(unique(merge$category)))/20)+0.55)*400
...
create_summary_forestplot(db, "events_ORR", "N_eval_eff_total", plot_name = "pred_ORR_year", plot_title = "ORR by year", category="year_group", use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_ORR", "N_eval_eff_total", plot_name = "pred_ORR_year_p1", plot_title = "ORR by year", phase=1, category="year_group", use_prediction_intervals = TRUE)  
create_summary_forestplot(db, "events_ORR", "N_eval_eff_total", plot_name = "pred_ORR_year_p2", plot_title ="ORR by year", phase=2, category="year_group", use_prediction_intervals = TRUE) 
create_summary_forestplot(db, "events_ORR", "N_eval_eff_total", plot_name = "pred_ORR_phase", plot_title = "ORR by clinical trial phase", category="Phase", use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_ORR", "N_eval_eff_total", plot_name = "pred_ORR_drugtype", plot_title = "ORR by tested compound category", category="DrugType",plot_complete = TRUE, use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_ORR", "N_eval_eff_total", plot_name = "pred_ORR_drugtype_p1", plot_title = "ORR by tested compound category", phase=1, category="DrugType", use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_ORR", "N_eval_eff_total", plot_name = "pred_ORR_drugtype_p2", plot_title = "ORR by tested compound category", phase=2, category="DrugType", use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_ORR", "N_eval_eff_total", plot_name = "pred_ORR_Investigational", plot_title = "ORR by investigational status", category="Investigational_status", use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_ORR", "N_eval_eff_total", plot_name = "pred_ORR_Investigational_p1", plot_title = "ORR by investigational status", phase=1, category="Investigational_status", use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_ORR", "N_eval_eff_total", plot_name = "pred_ORR_Investigational_p2", plot_title = "ORR by investigational status", phase=2, category="Investigational_status", use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_ORR", "N_eval_eff_total", plot_name = "pred_ORR_PS", plot_title = "ORR by performance status categories", category="PS_ratio010", use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_ORR", "N_eval_eff_total", plot_name = "pred_ORR_Targeted", plot_title = "ORR by if population was preselected or not", category="Targeted", use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_ORR", "N_eval_eff_total", plot_name = "pred_ORR_Targeted_p1", plot_title = "ORR by if population was preselected or not", category="Targeted", phase=1, use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_ORR", "N_eval_eff_total", plot_name = "pred_ORR_Targeted_p2", plot_title = "ORR by if population was preselected or not", category="Targeted", phase=2, use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_ORR", "N_eval_eff_total", plot_name = "pred_ORR_MSI", plot_title = "ORR by MSI ratio", category="MSI_20", use_prediction_intervals = TRUE)
create_summary_forestplot(long_MS, "ORR", "n_eva", plot_name = "ORR_MS", plot_title = "ORR by MS", category="MSS_MSI", use_prediction_intervals = TRUE)
...
create_summary_forestplot(db, "events_CBR", "N_eval_eff_total", plot_name = "pred_CBR_year", plot_title = "CBR by year", category="year_group", use_prediction_intervals = TRUE)  
create_summary_forestplot(db, "events_CBR", "N_eval_eff_total", plot_name = "pred_CBR_year_p1", plot_title = "CBR by year", phase=1, category="year_group", use_prediction_intervals = TRUE)  
create_summary_forestplot(db, "events_CBR", "N_eval_eff_total", plot_name = "pred_CBR_year_p2", plot_title = "CBR by year", phase=2, category="year_group", use_prediction_intervals = TRUE)  
create_summary_forestplot(db, "events_CBR", "N_eval_eff_total", plot_name = "pred_CBR_phase", plot_title = "CBR by clinical trial phase", category="Phase", use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_CBR", "N_eval_eff_total", plot_name = "pred_CBR_drugtype", plot_title = "CBR by tested compound category", category="DrugType",plot_complete = TRUE, use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_CBR", "N_eval_eff_total", plot_name = "pred_CBR_drugtype_p1", plot_title = "CBR by tested compound category", phase=1, category="DrugType", use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_CBR", "N_eval_eff_total", plot_name = "pred_CBR_drugtype_p2", plot_title = "CBR by tested compound category", phase=2, category="DrugType", use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_CBR", "N_eval_eff_total", plot_name = "pred_CBR_Investigational", plot_title = "CBR by investigational status", category="Investigational_status", Width=2700, use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_CBR", "N_eval_eff_total", plot_name = "pred_CBR_Investigational_p1", plot_title = "CBR by investigational status", phase=1, category="Investigational_status", use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_CBR", "N_eval_eff_total", plot_name = "pred_CBR_Investigational_p2", plot_title = "CBR by investigational status", phase=2, category="Investigational_status", use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_CBR", "N_eval_eff_total", plot_name = "pred_CBR_PS", plot_title = "CBR by performance status categories", category="PS_ratio010", use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_CBR", "N_eval_eff_total", plot_name = "pred_CBR_Targeted", plot_title = "CBR by if population was preselected or not", category="Targeted", use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_CBR", "N_eval_eff_total", plot_name = "pred_CBR_Targeted_p1", plot_title = "CBR by if population was preselected or not", category="Targeted", phase=1, use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_CBR", "N_eval_eff_total", plot_name = "pred_CBR_Targeted_p2", plot_title = "CBR by if population was preselected or not", category="Targeted", phase=2, use_prediction_intervals = TRUE)
create_summary_forestplot(db, "events_CBR", "N_eval_eff_total", plot_name = "pred_CBR_MSI", plot_title = "CBR by MSI raio", category="MSI_20", use_prediction_intervals = TRUE)  
create_summary_forestplot(long_MS, "CBR", "n_eva", plot_name = "CBR_MS", plot_title = "CBR by MS", category="MSS_MSI", use_prediction_intervals = TRUE)
