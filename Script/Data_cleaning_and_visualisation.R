# Data_cleaning_and_visualisation.R
# 
# This script processes and analyzes clinical trial data for metastatic colorectal cancer.
# It includes data cleaning, statistical analysis, and generation of various plots
# for efficacy and safety metrics across different trial phases and drug types.
#
# Last updated: 12-09-2024
# Authors: M.A. Huismans, L.P. Smabers
...
descriptive_plot_dir <- file.path(plot, "Descriptive")

# Function Definitions ----
histogram_function <- function(df, descriptive, bins=30, title=paste("Histogram of",descriptive)) {
...
# Load the preprocessed clinical trial data
# This data has already undergone initial cleaning steps and contains
# only the trials included in our paper. 

d_complete <- read_excel(file.path(input, "filtered_database.xlsx"))
...
d_string_complete_efficacy <- d_string_complete2 %>% dplyr::select(all_of(efficacy_cols), Phase) 

d_long_efficacy <- pivot_longer(d_string_complete_efficacy, cols=all_of(efficacy_cols), names_to = "column", values_to = "level")

d_string_complete_safety1 <- d_string_complete2 %>% dplyr::select(all_of(safety_cols1), Phase) 
...
d_string_complete_safety12 <- d_string_complete2 %>% dplyr::select(all_of(safety_cols1), all_of(safety_cols2), Phase) 
...
d_string_complete_safety2 <- d_string_complete2 %>% dplyr::select(all_of(safety_cols2)) 
...
# This includes creating new variables and categorizing existing ones

# Calculate percentages for efficacy data
...
d_complete$PS_ratio_cat <- factor(d_complete$PS_ratio_cat, levels = c("<0", "0-0.1", ">0.1")) 
...
d_complete$Max_age_cat <- cut(
  ...
  include.lowest = TRUE
) # make age categories based on distribution
...
histogram_function(d_complete, "Ratio_event_top5") 


#  Analysis ----
# Perform statistical analyses on the transformed data
# This includes correlations, regressions, and other statistical tests

## Efficacy availability plots ----
# These plots show the availability and distribution of efficacy metrics
...
## Safety availability plots ----
# These plots visualize safety-related data such as adverse events
...
# These plots show the distribution of various patient characteristics

p <- barplot_nocategory(d_complete, "Phase", title="Number of studies per Phase")
# ggsave(file.path(descriptive_plot_dir, "Phase.png"), plot=p, width=800, height=1500, units="px")

#raw data plots
medage <- barplot_nocategory(d_complete, "Med_age", title="Median age") #check distribution age median
maxage <- barplot_nocategory(d_complete, "Max_age", title="Maximum age") #check distribution age maximum
...
# ggsave(file.path(descriptive_plot_dir, "ps_age_lines.png"), plot=ps_age_lines, width=2500, height=3000, units="px")
...
#save plot
# ggsave(file.path(descriptive_plot_dir, "violin_med_age.png"), plot=violin_med_age ,width=1100, height=900, units="px")
...
#save plot
# ggsave(file.path(descriptive_plot_dir, "violin_max_age.png"), plot=violin_max_age ,width=1100, height=900, units="px")
...
#save plot
# ggsave(file.path(descriptive_plot_dir, "violin_max_lines.png"), plot=violin_max_lines ,width=1100, height=900, units="px")
...
#save plot
# ggsave(file.path(descriptive_plot_dir, "violin_min_lines.png"), plot=violin_min_lines ,width=1100, height=900, units="px")
...
#save plot
# ggsave(file.path(descriptive_plot_dir, "violin_med_lines.png"), plot=violin_med_lines ,width=1100, height=900, units="px")
...
#save plot
# ggsave(file.path(descriptive_plot_dir, "violin_year.png"), plot=violin_year ,width=1100, height=900, units="px")
...
crctr2 <- histogram_function(d_complete_ns, "CRC_total_ratio", bins=20, title="Histogram of \nCRC:All ratio\nNon-specific")
...
# ggsave(file.path(descriptive_plot_dir, "SpecCRC_eff.png"), plot=crc_spec_eff, width=2800, height=1650, units="px")
...
# ggsave(file.path(descriptive_plot_dir, "Treatment+YearGroup.png"), plot=dtyg,width=3000, height=2000, units="px")
...
tiff(file = file.path(descriptive_plot_dir, "SafetyMetrics.png"),
     units="in",
     width = (7),
     height = (5),
     res=300) 
...
## Correlation Plots ----
# These plots visualize correlations between different variables
...
# ggsave(file.path(descriptive_plot_dir, "Corplots.png"), plot=corplots,width=2700, height=2000, units="px")
...
ggsave(file.path(descriptive_plot_dir, "corplot_by_phase.png"), plot=corplot_by_phase, width=2000, height=2000, units="px")
...
corplot_dt
ggsave(file.path(descriptive_plot_dir, "Corplot_cbrvstop5_drugtype.pdf"), plot=corplot_dt, width=1800, height=1300, units="px")
...
ggsave(file.path(descriptive_plot_dir, "corplot_by_yg.png"), plot=corplot_ef_tox_yg, width=2000, height=2000, units="px")
ggsave(file.path(descriptive_plot_dir, "corplot_by_yg_phase.pdf"), plot=corplot_ef_tox_yg_phase, width=1800, height=1300, units="px")
...
d_long_string_complete_target <- d_string_complete %>% dplyr::select(all_of(targeted_cols)) %>% pivot_longer(cols=all_of(targeted_cols), names_to = "column", values_to = "level")
...
targeted_yeargroup <- ggplot(d_complete_ratio, aes(x = year_group, y = ratio, fill = targeted)) +
  ...
  theme_classic() + 
  labs(
    title = "Percentage of trials with or without \npatient preselection on molecular\ntraits by year group",
    x = "Year group",
    y = "Percentage",
    fill = "Preselection"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_manual(values = c("Targeted" = "#00A88E", "Not targeted" = "lightblue"),
                    labels = c("Preselected", "Not preselected")) + 
  scale_y_continuous(labels = scales::percent)
targeted_yeargroup

ggsave(file.path(descriptive_plot_dir, "preselection_by_yeargroup.pdf"), plot=targeted_yeargroup, width=1200, height=1200, units="px")

### year_group vs years extra plots ----
...
#save the plot
# ggsave(file.path(descriptive_plot_dir, "Targeted_YearGroup.png"), plot=targeted_yeargroup, width=1100, height=1200, units="px")

## investigational status plots -----

# 1. Calculate the ratio within each category of "Investigational_status" and "year_group"
...
rename_lookup <- data.frame(
  original_name = c("Investigational Monotherapy", 
                    "Combination of investigational and approved drugs", 
                    "Multiple investigational drugs"),
  new_name = c("Investigational drug monotherapy",
               "Combination of investigational\nand approved drugs",
               "Multiple investigational drugs")
)

# Step 2: Join to replace names
d_complete_ratio_investigation <- d_complete_ratio_investigation %>%
  left_join(rename_lookup, by = c("Investigational_status" = "original_name")) %>%
  dplyr::select(-Investigational_status) %>% 
  rename(Investigational_status = new_name) 

# 2. Modify labels to include a line break (using \n for newline) 
d_complete_ratio_investigation$Investigational_status <- 
  factor(d_complete_ratio_investigation$Investigational_status, 
         levels = c("Investigational drug monotherapy", 
                    "Combination of investigational\nand approved drugs",
                    "Multiple investigational drugs"))

# 3. Plot with the updated data - order will now match the factor levels
investigational_status_plot <- ggplot(d_complete_ratio_investigation, aes(x = year_group, y = ratio, fill = Investigational_status)) +
  geom_bar(stat="identity", position = "stack", color = "black") +
  theme_classic() +
  labs(
    title = "Percentage of investigational status \nby year group",
    x = "Year group",
    y = "Percentage",
...
# Save the plot
ggsave(file.path(descriptive_plot_dir, "Investigational_Status_YearGroup.pdf"), plot = investigational_status_plot, width=1700, height=1200, units="px")
...
# Rename categories & title using scale_fill_manual
ps_ratio_plot <- ggplot(d_complete_ratio_ps, aes(x = year_group, y = ratio, fill = PS_ratio_cat)) +
  ...
  theme_classic() +
  labs(
    title = "Percentage of patients with PS of 2\nor higher by year group",  # Update title
    x = "Year group",
    y = "Percentage",
    fill = "PS percentage\ncategories"
  ) +  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_fill_manual(values = c("lightblue", "#00A88E", "#C9A8CE", "white"),
                    labels = c("No PS 2 patients", "0-10% PS 2 patients", ">10% PS 2 patients", "Unknown"),
                    na.value = "white") +  # Add labels argument
  scale_y_continuous(labels = scales::percent) 

ps_ratio_plot

ggsave(file.path(descriptive_plot_dir, "ps2_by_yeargroup.pdf"), plot=ps_ratio_plot, width=1200, height=1200, units="px")

# Filter out "Unknown" and reorder factor levels
d_complete_ratio_ps_filtered <- d_complete_ratio_ps %>%
  filter(PS_ratio_cat != "Unknown") %>%
  mutate(PS_ratio_cat = factor(PS_ratio_cat, levels = c("<0", "0-0.1", ">0.1")))

# Create the plot
ps_ratio_plot_filtered <- ggplot(d_complete_ratio_ps_filtered, aes(x = year_group, y = ratio, fill = PS_ratio_cat)) +
  geom_bar(stat="identity", position = position_fill(reverse=TRUE), color = "black") +
  theme_classic() +
  labs(
    title = "Percentage of patients with PS of 2\nor higher by year group", 
    x = "Year group",
    y = "Percentage",
    fill = "PS percentage\ncategories"
  ) +  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_fill_manual(values = c("#C9A8CE", "#00A88E", "lightblue"),
                    labels = c("No PS 2 patients", "0-10% PS 2 patients", ">10% PS 2 patients")) + 
  scale_y_continuous(labels = scales::percent) 

ps_ratio_plot_filtered
ggsave(file.path(descriptive_plot_dir, "ps2_by_yeargroup_no_unknown.pdf"), plot=ps_ratio_plot_filtered, width=1200, height=1200, units="px")
...
# Save the plot
# ggsave(file.path(descriptive_plot_dir, "PS_Ratio_Category_YearGroup.png"), plot = ps_ratio_plot, width=1200, height=1200, units="px")

## med age per year group plots ----
...
med_age_plot <- ggplot(d_complete_ratio_age, aes(x = year_group, y = ratio, fill = Med_age_cat)) +
  ...
  theme_classic() +
  labs(
    title = "Percentage of median age category \nby year group",
    x = "Year group",
    y = "Percentage",
...
ggsave(file.path(descriptive_plot_dir, "median_age_by_yeargroup.pdf"), plot=med_age_plot, width=1200, height=1200, units="px")

d_complete_ratio_age_filtered <- d_complete_ratio_age %>%
  filter(!is.na(Med_age_cat))

med_age_plot_filtered <- ggplot(d_complete_ratio_age_filtered, aes(x = year_group, y = ratio, fill = Med_age_cat)) +
  geom_bar(stat="identity", position = position_fill(reverse=TRUE), color = "black") +
  theme_classic() +
  labs(
    title = "Percentage of median age of a trial within \nage categories by year group",
    x = "Year group",
    y = "Percentage",
    fill = "Median age category"
  ) +  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_fill_manual(values = c("#C9A8CE", "#00A88E", "lightblue", "white")) +
  scale_y_continuous(labels = scales::percent) # You can add your custom colors using scale_fill_manual if needed

med_age_plot_filtered

ggsave(file.path(descriptive_plot_dir, "median_age_by_yeargroup_no_missing.pdf"), plot=med_age_plot_filtered, width=1200, height=1200, units="px")

histogram_function(d_complete, "Med_age") # approaches normal distribution, so it's okay to use the mean of medians!
...
# Save the plot
# ggsave(file.path(descriptive_plot_dir, "Med_Age_Category_YearGroup.png"), plot = med_age_plot, width=1200, height=1200, units="px")

## Stacked Bar Plots ----
# These plots show the composition of different categories over time or by groups
# Calculate the ratio within each category of "PS_ratio_cat" and "year_group"
...
# Correctly create the factor with matching labels
d_complete_ratio_drug$DrugType <-  factor(d_complete_ratio_drug$DrugType, 
                                          levels = c("Vaccine", "Chemotherapy", "Antiangiogenesis", "Receptor or signal transduction", "Immunomodulator", "Other")) 
...
drugtype_year_plot <- ggplot(d_complete_ratio_drug, aes(x = year_group, y = ratio, fill = DrugType)) +
  ...
  theme_classic() +
  labs(
    title = "Percentage of trials testing a drug from \na drug category by year group",
    x = "Year group",
    y = "Percentage",
    fill = "Drug category"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_fill_manual(values = c("white", "lightblue", "#00A88E", "#C9A8CE",  "#918EC9", "#E68469")) +
  scale_y_continuous(labels = scales::percent) # You can add your custom colors using scale_fill_manual if needed
...
ggsave(file.path(descriptive_plot_dir, "drugtype_by_yeargroup.pdf"), plot=drugtype_year_plot, width=1700, height=1200, units="px")
...
# ggsave(file.path(descriptive_plot_dir, "DrugType_Category_YearGroup.png"), plot = drugtype_year_plot, width=1300, height=1100, units="px")
...
# ggsave(file.path(descriptive_plot_dir, "DrugType_Category_investigationalstatus.png"), plot = drugtype_investigational_plot, width=1700, height=1200, units="px")
...
# ggsave(file.path(descriptive_plot_dir, "Phase_Category_investigationalstatus.png"), plot = phase_investigational_plot, width=1500, height=1200, units="px")
...
# ggsave(file.path(descriptive_plot_dir, "Phase_Category_DrugType.png"), plot = phase_drugtype_plot, width=1200, height=1200, units="px")
