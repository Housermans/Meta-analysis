...
create_summary_forestplot <- function(data, events_col, n_col,  plot_name, plot_title, category, phase = NULL, Width = 1700, plot_complete = FALSE, use_prediction_intervals = FALSE) {
  ...
  plot <- plot_grid(
    ...
    nrow = 1,
    rel_heights = c(1),
    rel_widths = c(((((max(stringr::str_width(unique(merge$category))))/20)+0.55)*4), 2.5, 3.5, 5)
  ) +
...
plot
  ...
  ggsave(file.path(plot_dir_sum, paste0(plot_name, "NVN_summary.pdf")), plot=plot, width=Width, height=Height, units="px")
