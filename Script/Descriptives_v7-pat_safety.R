...
d <- read_excel(file.path(input, "databaseV230926_2.xlsx"))
...
write_xlsx(d_top5, file.path(output, "filtered_db_230923_2.xlsx"))
...
d_complete_ratio_yeargroup <- d_complete %>%
...
targeted_yeargroup <- ggplot(d_complete_ratio_yeargroup, aes(x = year_group, y = ratio, fill = targeted)) +
  geom_bar(stat="identity", position = "stack", color = "black") +
  theme_classic() + 
  labs(
    title = "Ratio of Targeted and Not Targeted by Year Group",
    x = "Year Group",
    y = "Ratio",
    fill = "Target"
  ) +
...
targeted_yeargroup
ggsave(file.path(plot_d, "Targeted_YearGroup.png"), plot=targeted_yeargroup)

d_complete_ratio_drugtype <- d_complete %>%
  count(DrugType, targeted) %>%
  group_by(DrugType) %>%
  mutate(ratio = n / sum(n)) %>%
  ungroup()

targeted_drugtype <- ggplot(d_complete_ratio_drugtype, aes(x = DrugType, y = ratio, fill = targeted)) +
  geom_bar(stat="identity", position = "stack", color = "black") +
  theme_classic() + theme(axis.text.x= element_text(angle=45, vjust=1, hjust=1, size=12)) + 
  labs(
    title = "Ratio of Targeted and Not Targeted by treatment type",
    x = "Treatment type",
    y = "Ratio",
    fill = "Target"
  ) +
  ...
  scale_y_continuous(labels = scales::percent)
targeted_drugtype
ggsave(file.path(plot_d, "Targeted_DrugType.png"), plot=targeted_drugtype, height = 5)
...
investigational_status_plot <- ggplot(d_complete_ratio_investigation, aes(x = year_group, y = ratio, fill = Investigational_status)) +
  ...
  theme_classic() +
  labs(
    title = "Ratio of Investigational Status by Year Group",
    x = "Year Group",
    y = "Ratio",
...
# Save the plot
ggsave(file.path(plot_d, "Investigational_Status_YearGroup.png"), plot = investigational_status_plot)
...
ps_ratio_plot <- ggplot(d_complete_ratio_ps, aes(x = year_group, y = ratio, fill = PS_ratio_cat)) +
  ...
  theme_classic() +
  labs(
    title = "Ratio of PS Ratio Category by Year Group",
    x = "Year Group",
    y = "Ratio",
...
# Save the plot
ggsave(file.path(plot_d, "PS_Ratio_Category_YearGroup.png"), plot = ps_ratio_plot)
...
med_age_plot <- ggplot(d_complete_ratio_age, aes(x = year_group, y = ratio, fill = Med_age_cat)) +
  ...
  theme_classic() +
  labs(
    title = "Ratio of Median Age Category by Year Group",
    x = "Year Group",
    y = "Ratio",
...
# Save the plot
ggsave(file.path(plot_d, "Med_Age_Category_YearGroup.png"), plot = med_age_plot)
...
drugtype_year_plot <- ggplot(d_complete_ratio_drug, aes(x = year_group, y = ratio, fill = DrugType)) +
  ...
  theme_classic() +
  labs(
    title = "Ratio of DrugType by Year Group",
    x = "Year Group",
    y = "Ratio",
...
ggsave(file.path(plot_d, "DrugType_Category_YearGroup.png"), plot = drugtype_year_plot)
...
drugtype_investigational_plot <- ggplot(d_complete_ratio_drug_investigational, aes(x = DrugType, y = ratio, fill = Investigational_status)) +
  ...
  theme_classic() +
  labs(
    title = "Ratio of DrugType by investigational status",
    x = "Drugtype",
    y = "Ratio",
    fill = "Investigational status"
  ) +
...
ggsave(file.path(plot_d, "DrugType_Category_investigationalstatus.png"), plot = drugtype_investigational_plot)
