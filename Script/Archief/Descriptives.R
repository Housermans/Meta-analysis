...
d <- read_excel(file.path(input, "databaseV230310.xlsx"))
...
d_string <- d
d_string_complete <- d_string[d_string$Gevuld > 0.86,] # alleen artikelen die afgemaakt zijn 
...
d_complete <- d[d$Gevuld > 0.86,] # alleen artikelen die afgemaakt zijn 
d_removed <- d[d$Gevuld < 0.86,]

change_to_numeric <- c("PFS_med", "PFS_CRC", "uPR")
d_complete <- mutate_at(d_complete, vars(all_of(change_to_numeric)), as.numeric)
d_complete <- mutate_at(d_complete, vars(starts_with("N_")), as.numeric)
d_complete <- mutate_at(d_complete, vars(starts_with("T_")), as.numeric)

d_complete <- d_complete %>% mutate(
                              CRC_total_ratio = N_inc_crc/N_inc,
                              CBR = (N_SD_short + cPR + CR) / N_eva_eff,
                              CBR_CRC = (N_SD_short + cPR + CR) / N_eva_eff_crc,
                              ORR = (cPR + CR) / N_eva_eff,
                              ORR_CRC = (cPR + CR) / N_eva_eff_crc,
                              CBR_all = (pmax(N_SD_short, N_SD_long, na.rm=TRUE) + cPR + CR) / N_eva_eff,
                              CBR_CRC_all = (pmax(N_SD_short, N_SD_long, na.rm=TRUE) + cPR + CR) / N_eva_eff_crc, 
                              events_CBR = pmax(N_SD_short, N_SD_long, na.rm=TRUE) + cPR + CR, 
                              events_ORR = cPR + CR,
...
write_xlsx(d_complete, file.path(output, "filtered_db3.xlsx"))
...
convert <- c("N_NE", "N_PD", "N_SD_short", "N_SD_long", "uPR", "cPR", "CR", "PFS_med", 
             "N_DLT", "N_red", "N_SAE", "N_SAE_rel", "Dose_Del_Int", "N_Disc", "N_pat_GR3+", "N_pat_Gr3", "N_pat_Gr4", "N_patGr5", 
             "N_events_safety", "N_events_Gr3+", "N_events_Gr4", "N_events_Gr5")
add_level <- function(x, y) { factor(x, levels = c(levels(x), y)) }

d_string_complete2 <- d_string_complete %>% mutate(across(all_of(convert), ~factor(., levels = c("NA", "NR"), 
                                                                                   labels = c("Not Applicable", "Not Reported"))))
d_string_complete2 <- d_string_complete %>% 
  mutate(across(all_of(convert), ~factor(., levels = c("NA", "NR"), labels = c("Not Applicable", "Not Reported")))) %>% 
...
efficacy_cols <- c("N_NE", "N_PD", "N_SD_short", "N_SD_long", "uPR", "cPR", "CR", "PFS_med")
safety_cols1 <- c("N_DLT", "N_red", "N_SAE", "N_SAE_rel", "Dose_Del_Int", "N_Disc")
safety_cols2 <- c("N_pat_GR3+", "N_pat_Gr3", "N_pat_Gr4", "N_patGr5", "N_events_safety", "N_events_Gr3+", "N_events_Gr4", "N_events_Gr5")
...
safe1 <- ggplot(d_long_safety1, aes(x = column, fill = level)) + 
  ...
  scale_x_discrete(labels = c("# patients dose delay or interruption", "# patients drug discontinuation", "# patients DLT", "# patients dose reduction", "# patients SAE", 
                              "# patients related SAE")) + 
  xlab("")
...
safe2 <- ggplot(d_long_safety2, aes(x = column, fill = level)) + 
  geom_bar(position = "stack") + theme_classic() + 
  theme(axis.text.x= element_text(angle=40, vjust=1, hjust=1, size=12)) + 
  scale_x_discrete(labels = c("# events GR3 and higher", "# events GR3", "# events GR4", "# events GR5", "total N safety events reported", 
                                "# patients with Grade 3 AE", "# patients with Grade 3 or higher AE", "# patients Grade 4 AE", "# patients Grade 5 AE")) + 
  xlab("")
...
reason <- barplot(d_removed, "ProblemClass", title="Reason of exclusion")
