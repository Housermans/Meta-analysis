...
db <- read_excel(file.path(output, "filtered_db_230508.xlsx"))
...
DLT <- db_p1$N_DLT
disc <- db_p2$N_Disc
gr3 <- db_safety_events$`N_events_Gr3+`
studlab_safety_events <- paste(db_safety_events$Aut_1, db_safety_events$Year_published_online, sep = ", ")
studlab_p1 <- paste(db_p1$Aut_1, db_p1$Year_published_online, sep = ", ")
studlab_p2 <- paste(db_p2$Aut_1, db_p2$Year_published_online, sep = ", ")

#plots----------------------
#plot CBR
CBR <- metaprop(events_CBR, patients, studlab, sm = "PLOGIT", comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5))
forest_CBR <- forest(CBR)
rstudioapi::savePlotAsImage(file.path(plot, "CBR.png"), width = 800, height = 1500)
CBR$lower
...
rstudioapi::savePlotAsImage(file.path(plot, "ORR.png"), width = 800, height = 1500)

#plot grade 3 + toxicities
GR3 <- metaprop(gr3, safety_events, studlab_safety_events, sm = "PLOGIT", comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5))
forest_GR3 <- forest(GR3)
rstudioapi::savePlotAsImage(file.path(plot, "GR3.png"), width = 800, height = 1000)

#plot DLT toxicity (phas 1)
DLT <- metaprop(DLT, patients_safety_p1, studlab_p1, sm = "PLOGIT", comb.fixed=FALSE, comb.random=TRUE, 
                method = "Inverse", control=list(stepadj=0.5))
forest_DLT <- forest(DLT)
rstudioapi::savePlotAsImage(file.path(plot, "DLT.png"), width = 800, height = 1000)

#plot discontinuation toxicity (phase 2)
disc <- metaprop(disc, patients_safety_p2, studlab_p2, sm = "PLOGIT", comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5))
forest_disc <- forest(disc)
rstudioapi::savePlotAsImage(file.path(plot, "disc.png"), width = 800, height = 1000)
...
rstudioapi::savePlotAsImage(file.path(plot, "CBR_year.png"), width = 800, height = 1900)

library(metafor)
Estimate<-transf.ilogit(CBR_year$TE.random.w,ni=CBR_year$n.harmonic.mean.w) #back transformation logit
Upper<-transf.ilogit(CBR_year$lower.random.w,ni=CBR_year$n.harmonic.mean.w)
Lower<-transf.ilogit(CBR_year$upper.random.w,ni=CBR_year$n.harmonic.mean.w)
...
#plot CBR subgroup
CBR_subgroup <- metaprop(events_CBR, patients, byvar = drugtype, studlab, sm = "PLOGIT", 
                comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5),
                prediction.subgroup=TRUE, test.subgroup=TRUE, overall = FALSE, overall.hetstat = FALSE)
forest_CBR_subrgoup <- forest(CBR_subgroup)
rstudioapi::savePlotAsImage(file.path(plot, "CBR_subgroup_v2.png"), width = 800, height = 2500)

Estimate<-transf.ilogit(CBR_subgroup$TE.random.w,ni=CBR_subgroup$n.harmonic.mean.w) #back transformation logit
Upper<-transf.ilogit(CBR_subgroup$lower.random.w,ni=CBR_subgroup$n.harmonic.mean.w)
Lower<-transf.ilogit(CBR_subgroup$upper.random.w,ni=CBR_subgroup$n.harmonic.mean.w)
