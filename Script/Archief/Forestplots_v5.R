...
db <- read_excel(file.path(output, "filtered_db_230801.xlsx"))
...
db_events_ORR <- db %>% filter(!is.na(db$events_ORR))
...
db_events_CBR_p1 <- db_events_CBR %>% filter(Phase == "1")
db_events_CBR_p2 <- db_events_CBR %>% filter(Phase == "2")
db_events_ORR_p1 <- db_events_ORR %>% filter(Phase == "1")
db_events_ORR_p2 <- db_events_ORR %>% filter(Phase == "2")
...
Forestplot_function <- function(db, events, total_patients, studylabel, title, width, height) {
  metaprop <- metaprop(db, get(events), get(total_patients), get(studylabel), sm = "PLOGIT", comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5))
  forestplot <- forest(metaprop)
  rstudioapi::savePlotAsImage(file.path(plot, paste0(title, ".png")), width = width, height = height)}

Forestplot_function_category <- function(db, events, total_patients, studylabel, category, title, width, height){
  metaprop <- metaprop(db, events, total_patients, category, studylabel, sm = "PLOGIT", comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5))
  forestplot <- forest(metaprop)
  rstudioapi::savePlotAsImage(file.path(plot, paste0(title, ".png")), width = width, height = height)}

db$studlab
Forestplot_function(db_events_CBR, "events_CBR", "N_eval_eff_total", "studlab", "CBR", 800, 2000)
Forestplot_function(db, events_ORR, N_eval_eff_total, studlab, "ORR", 800, 4500)
Forestplot_function(db_p1, N_DLT, N_pat_safety, studlab, "DLT", 800, 3000)
Forestplot_function(db_p2_disc, N_Disc, N_pat_safety, studlab, "disc", 800, 1200)
Forestplot_function(db_top5, events_CBR, N_eval_eff_total, studlab, "CBR")
Forestplot_function(db_events_CBR, events_CBR, N_eval_eff_total, studlab, "CBR")
Forestplot_function(db_events_CBR, events_CBR, N_eval_eff_total, studlab, "CBR")
...
ORR_year <- metaprop(db_events_ORR$events_ORR, db_events_ORR$N_eval_eff_total, byvar = db_events_ORR$year_group, db_events_ORR$studlab, sm = "PLOGIT", 
...
ORR_year_p1 <- metaprop(db_events_ORR_p1$events_ORR, db_events_ORR_p1$N_eval_eff_total, byvar = db_events_ORR_p1$year_group, db_events_ORR_p1$studlab, sm = "PLOGIT", 
                     comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5),
                     prediction.subgroup=TRUE, test.subgroup=TRUE, overall = FALSE, overall.hetstat = FALSE)
forest_ORR_year_p1 <- forest(ORR_year_p1)
rstudioapi::savePlotAsImage(file.path(plot, "ORR_year_p1.png"), width = 800, height = 3500)

Estimate<-transf.ilogit(ORR_year_p1$TE.random.w) #back transformation logit
Upper<-transf.ilogit(ORR_year_p1$upper.random.w)
Lower<-transf.ilogit(ORR_year_p1$lower.random.w)
#RUN HIERNA VANAF PUNT Y OM EEN SUMMARY PLOT TE MAKEN EN DENK AAN AANPASSEN NAAM OM OP TE SLAAN

ORR_year_p2 <- metaprop(db_events_ORR_p2$events_ORR, db_events_ORR_p2$N_eval_eff_total, byvar = db_events_ORR_p2$year_group, db_events_ORR_p2$studlab, sm = "PLOGIT", 
                        comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5),
                        prediction.subgroup=TRUE, test.subgroup=TRUE, overall = FALSE, overall.hetstat = FALSE)
forest_ORR_year_p2 <- forest(ORR_year_p2)
rstudioapi::savePlotAsImage(file.path(plot, "ORR_year_p2.png"), width = 800, height = 3500)

Estimate<-transf.ilogit(ORR_year_p2$TE.random.w) #back transformation logit
Upper<-transf.ilogit(ORR_year_p2$upper.random.w)
Lower<-transf.ilogit(ORR_year_p2$lower.random.w)
#RUN HIERNA VANAF PUNT Y OM EEN SUMMARY PLOT TE MAKEN EN DENK AAN AANPASSEN NAAM OM OP TE SLAAN

#plot CBR subgroup year
CBR_inv <- metaprop(db_events_CBR$events_CBR, db_events_CBR$N_eval_eff_total, byvar = db_events_CBR$Investigational_status, db_events_CBR$studlab, sm = "PLOGIT", 
                    comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5),
                    prediction.subgroup=TRUE, test.subgroup=TRUE, overall = FALSE, overall.hetstat = FALSE)
forest_CBR_inv <- forest(CBR_inv)
rstudioapi::savePlotAsImage(file.path(plot, "CBR_inv.png"), width = 800, height = 4000)

Estimate<-transf.ilogit(CBR_inv$TE.random.w) #back transformation logit
Upper<-transf.ilogit(CBR_inv$upper.random.w)
Lower<-transf.ilogit(CBR_inv$lower.random.w)

CBR_year <- metaprop(db_events_CBR$events_CBR, db_events_CBR$N_eval_eff_total, byvar = db_events_CBR$year_group, db_events_CBR$studlab, sm = "PLOGIT", 
                         comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5),
                         prediction.subgroup=TRUE, test.subgroup=TRUE, overall = FALSE, overall.hetstat = FALSE)
...
CBR_year_p1 <- metaprop(db_events_CBR_p1$events_CBR, db_events_CBR_p1$N_eval_eff_total, byvar = db_events_CBR_p1$year_group, db_events_CBR_p1$studlab, sm = "PLOGIT", 
                     comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5),
                     prediction.subgroup=TRUE, test.subgroup=TRUE, overall = FALSE, overall.hetstat = FALSE)
forest_CBR_year_p1 <- forest(CBR_year_p1)
rstudioapi::savePlotAsImage(file.path(plot, "CBR_year_p1.png"), width = 800, height = 3500)

library(metafor)
Estimate<-transf.ilogit(CBR_year_p1$TE.random.w) #back transformation logit
Upper<-transf.ilogit(CBR_year_p1$upper.random.w)
Lower<-transf.ilogit(CBR_year_p1$lower.random.w)

CBR_year_p2 <- metaprop(db_events_CBR_p2$events_CBR, db_events_CBR_p2$N_eval_eff_total, byvar = db_events_CBR_p2$year_group, db_events_CBR_p2$studlab, sm = "PLOGIT", 
                        comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5),
                        prediction.subgroup=TRUE, test.subgroup=TRUE, overall = FALSE, overall.hetstat = FALSE)
forest_CBR_year_p2 <- forest(CBR_year_p2)
rstudioapi::savePlotAsImage(file.path(plot, "CBR_year_p1.png"), width = 800, height = 3500)

library(metafor)
Estimate<-transf.ilogit(CBR_year_p2$TE.random.w) #back transformation logit
Upper<-transf.ilogit(CBR_year_p2$upper.random.w)
Lower<-transf.ilogit(CBR_year_p2$lower.random.w)
...
tiff(file.path(plot, "inv_subanalysis_CBR.tiff"), #CHANGE NAME OF PLOT
     units="in",
     width = (4),
     height = (2),
...
#
#plot top 5 toxicity (phase 1)
top5_phase1_plot <- metaprop(db_p1_top5$`N_events_top5_Gr3+`, db_p1_top5$safetytimes5,byvar = db_p1_top5$DrugType, db_p1_top5$studlab, sm = "PLOGIT", comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5))
forest_top5_phase1 <- forest(top5_phase1_plot)
rstudioapi::savePlotAsImage(file.path(plot, "top5_phase1.png"), width = 800, height = 4000)

Estimate<-transf.ilogit(top5_phase1_plot$TE.random.w) #back transformation logit
Upper<-transf.ilogit(top5_phase1_plot$upper.random.w)
Lower<-transf.ilogit(top5_phase1_plot$lower.random.w)

top5_phase2_plot <- metaprop(db_p2_top5$`N_events_top5_Gr3+`, db_p2_top5$safetytimes5, byvar = db_p2_top5$DrugType, db_p2_top5$studlab, sm = "PLOGIT", comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5))
forest_top5_phase2 <- forest(top5_phase2_plot)
rstudioapi::savePlotAsImage(file.path(plot, "top5_phase2.png"), width = 800, height = 4000)

Estimate<-transf.ilogit(top5_phase2_plot$TE.random.w) #back transformation logit
Upper<-transf.ilogit(top5_phase2_plot$upper.random.w)
Lower<-transf.ilogit(top5_phase2_plot$lower.random.w)

#plot ORR subgroup year
ORR_year <- metaprop(db_events_ORR$events_ORR, db_events_ORR$N_eval_eff_total, byvar = db_events_ORR$DrugType, db_events_ORR$studlab, sm = "PLOGIT", 
                     comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5),
                     prediction.subgroup=TRUE, test.subgroup=TRUE, overall = FALSE, overall.hetstat = FALSE)
forest_ORR_year <- forest(ORR_year)
rstudioapi::savePlotAsImage(file.path(plot, "ORR_year.png"), width = 800, height = 3500)
...
ORR_year_p1 <- metaprop(db_events_ORR_p1$events_ORR, db_events_ORR_p1$N_eval_eff_total, byvar = db_events_ORR_p1$DrugType, db_events_ORR_p1$studlab, sm = "PLOGIT", 
                        comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5),
                        prediction.subgroup=TRUE, test.subgroup=TRUE, overall = FALSE, overall.hetstat = FALSE)
forest_ORR_year_p1 <- forest(ORR_year_p1)
rstudioapi::savePlotAsImage(file.path(plot, "ORR_year_p1.png"), width = 800, height = 3500)
...
ORR_year_p2 <- metaprop(db_events_ORR_p2$events_ORR, db_events_ORR_p2$N_eval_eff_total, byvar = db_events_ORR_p2$DrugType, db_events_ORR_p2$studlab, sm = "PLOGIT", 
                        comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5),
                        prediction.subgroup=TRUE, test.subgroup=TRUE, overall = FALSE, overall.hetstat = FALSE)
forest_ORR_year_p2 <- forest(ORR_year_p2)
rstudioapi::savePlotAsImage(file.path(plot, "ORR_year_p2.png"), width = 800, height = 3500)
...
#plot CBR subgroup 
CBR_year_p1 <- metaprop(db_events_CBR_p1$events_CBR, db_events_CBR_p1$N_eval_eff_total, byvar = db_events_CBR_p1$DrugType, db_events_CBR_p1$studlab, sm = "PLOGIT", 
                        comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5),
                        prediction.subgroup=TRUE, test.subgroup=TRUE, overall = FALSE, overall.hetstat = FALSE)
forest_CBR_year_p1 <- forest(CBR_year_p1)
rstudioapi::savePlotAsImage(file.path(plot, "CBR_year_p1.png"), width = 800, height = 3500)
...
CBR_year_p2 <- metaprop(db_events_CBR_p2$events_CBR, db_events_CBR_p2$N_eval_eff_total, byvar = db_events_CBR_p2$DrugType, db_events_CBR_p2$studlab, sm = "PLOGIT", 
                        comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5),
                        prediction.subgroup=TRUE, test.subgroup=TRUE, overall = FALSE, overall.hetstat = FALSE)
forest_CBR_year_p2 <- forest(CBR_year_p2)
rstudioapi::savePlotAsImage(file.path(plot, "CBR_year_p1.png"), width = 800, height = 3500)
...
CBR_subgroup_noNA <- metaprop(db_events_CBR$events_CBR, db_events_CBR$N_eval_eff_total, byvar = db_events_CBR$DrugType, db_events_CBR$studlab, sm = "PLOGIT", 
                         comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5),
                         prediction.subgroup=TRUE, test.subgroup=TRUE, overall = FALSE, overall.hetstat = FALSE)
...
tiff(file.path(plot, "drugs_subanalysis_p2_top5.tiff"),
     units="in",
     width = (6),
     height = (2.7),
     res=300) # The height of the plot in inches
...
#plot ORR subgroup
ORR_subgroup <- metaprop(db$events_ORR, db_year$studlab, byvar = db$DrugType, db$studlab, sm = "PLOGIT", 
                              comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5),
                              prediction.subgroup=TRUE, test.subgroup=TRUE, overall = FALSE, overall.hetstat = FALSE)
...
DLT_subgroup <- metaprop(db_p1$N_DLT, db_p1$N_pat_safety, byvar = db_p1$DrugType, db_p1$studlab, sm = "PLOGIT", 
                         comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5),
                         prediction.subgroup=TRUE, test.subgroup=TRUE, overall = FALSE, overall.hetstat = FALSE)
...
disc_subgroup <- metaprop(db_p2$N_Disc, db_p2$N_pat_safety, byvar = db_p2$DrugType, db_p2$studlab, sm = "PLOGIT", 
                         comb.fixed=FALSE, comb.random=TRUE, method = "Inverse", control=list(stepadj=0.5),
                         prediction.subgroup=TRUE, test.subgroup=TRUE, overall = FALSE, overall.hetstat = FALSE)
