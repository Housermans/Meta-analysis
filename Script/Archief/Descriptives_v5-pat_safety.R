...
d <- read_excel(file.path(input, "databaseV230601_dt2.xlsx"))
...
write_xlsx(d_complete, file.path(output, "filtered_db_230601.xlsx"))
...
medage <- barplot(d_complete, "Med_age", title="Median age")
maxage <- barplot(d_complete, "Max_age", title="Maximum age")
...
inex <- mps + minl + medl + maxl
age <- medage + maxage

ggsave(file.path(plot_d, "MAXPS.png"), plot=mps, width=2000, height=2000, units="px")
ggsave(file.path(plot_d, "inex.png"), plot=inex, width=3000, height=3000, units="px")
ggsave(file.path(plot_d, "age.png"), plot=age, width=3000, height=1200, units="px")
