...
create_summary_forestplot <- function(data, events_col, n_col,  plot_name, plot_title, category, phase = NULL, Width = 1700, plot_complete = FALSE, use_prediction_intervals = FALSE) {
  data <- db
  events_col <- "events_CBR"
  n_col <- "N_eval_eff_total"
  plot_name <-"CBR_year"
  plot_title <- "CBR by year"
  category <-  "year_group"
  phase <- NULL
  Width <- 1700
  plot_complete <- FALSE
  ...
  t1 <- ggplot(data = merge) +
    geom_text(aes(y = reorder(category, -index), x = 1, label = ifelse(is.na(Estimate), "", sprintf("%0.2f", Estimate*100))), vjust = 0) +
    geom_hline(yintercept = 11.6, linewidth = 2) +
    ggtitle("Proportion") +
    xlab("  ") +
    theme_classic(base_size = 9) +
  ...
  t2 <- ggplot(data = merge) +
    geom_text(aes(y = reorder(category, -index), x = 1, label = sprintf("(%0.2f; %0.2f)", Lower*100, Upper*100)), vjust = 0) +
    geom_hline(yintercept = 11.6, linewidth = 2) +
    ggtitle("95%-CI") +
    theme_classic(base_size = 9) +
    xlab("  ") +
  ...
  ggsave(file.path(plot_dir_sum, paste0(plot_name, "MAYEARS_summary.pdf")), plot=plot, width=Width, height=Height, units="px")
  plot
...
MA_II_years <- metaprop(event = events_CBR, n = N_eval_eff_total,
                        data = db,
                        method = "Inverse",
                        sm = gs("smprop"),
                        incr = gs("incr"),
